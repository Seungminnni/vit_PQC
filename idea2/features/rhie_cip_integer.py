...
def encode_integer(
    ...
    chunks = [base]
    for value in values:
        residual = torch.remainder(b_exp - int(value) * A, q)
        r_c = centered_float(residual, q)
        view = [r_c, r_c.abs(), *multi_phase_channels(residual, q, freqs[:1])]
        chunks.append(torch.stack(view, dim=1))
