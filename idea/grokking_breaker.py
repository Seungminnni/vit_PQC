...
# [처방 1: 모델 두뇌 파이프라인 2배 확장 (256 -> 512)]
class LWEIntegerViT(nn.Module):
    def __init__(self, M=512, n=16, embed_dim=512, num_heads=8, depth=6): # embed_dim 512로 확장
        super().__init__()
        self.n = n
...
def main():
    ...
    # [처방 2: 클래스 가중치 적용 - 0을 맞추면 점수 찔끔, 다른 숫자 맞추면 대박 보상]
    # 인덱스:  0   1   2   3(0)  4   5   6
    # 숫자:   -3  -2  -1   0    1   2   3
    ...
    print(f"🔥 [System] Sparse LWE 모드 V2 (클래스 가중치 + 512 차원)")
    ...
    for epoch in range(100):
        ...
        with torch.no_grad():
            ...
            if acc == 100.0:
                print("✨ 드디어 뚫었습니다! 이제 노이즈를 섞어도 됩니다.")
                break
